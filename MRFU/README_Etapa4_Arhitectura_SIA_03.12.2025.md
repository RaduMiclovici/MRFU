...
**Student:** Miclovici Radu-Stefan 
**Link Repository GitHub** https://github.com/RaduMiclovici/MRFU
...
|---------------------------|--------------------------------|--------------------------------|
| Recunoasterea animalelor|Trece imaginea prin rețea neuronală pentru clasificare| Modul RN + UI |
| Salvarea intr-o baza de date | Stocarea datelor procesate în timp real cu acces rapid | Modul Bază de Date |
| Achizitia datelor precum locatie etc| Analizeaza meta-data pentru a retine unde a fost vazut animalul  | Modul Achizitie Date |
...
```
Am achizitionat pozele originale de pe internet sursa stock.com si le-am preprocesat pentru a crea un dataset nou pentru clasificarea animalelor.
```
...
| **Etichetare/adnotare manuală** | • Etichetat manual 100+ imagini | Procesate prin script propriu pentru resize | Impartirea in train/val/test + cod de procesare|
...
**Descriere detaliată:**
Am etichetat manual peste 100 de imagini cu animale, folosind un script propriu pentru redimensionarea și preprocesarea imaginilor. Această etichetare manuală este esențială pentru antrenarea rețelei neuronale care clasifică animalele în funcție de speciile lor. Procesul a inclus verificarea calității imaginilor și asigurarea unei distribuții echilibrate între clase.

**Locația codului:** `src/preprocessing/preprocessing.py`
**Locația datelor:** `data/train/`, `data/validation/`
...
- **Formate acceptate:** JPG
- **Locație:** `docs/state_machine.*` (orice extensie)
- **Legendă obligatorie:** 
"De ce ați ales acest State Machine pentru nevoia voastră?"
Pentru a fi utilizat la proiectul de licenta unde o drona se va baza pe acest ai pentru recunoasterea animalului, logarea datelor de locatie si pattern al animalului, transmiterea catre o baza de date unde un utilizator se poate folosi de aceste informatii pentru a monitoriza fauna urbana. 

**Stări tipice pentru un SIA:**
```
IDLE → ACQUIRE_DATA → PREPROCESS → CLASSIFY → DISPLAY/ACT → LOG → [ERROR] → STOP
